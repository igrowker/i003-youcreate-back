from datetime import (
    date,
    timedelta,
)  # Importa funciones para manejar fechas y periodos de tiempo.
from decimal import (
    Decimal,
    ROUND_HALF_UP,
)  # Importa herramientas para manejar números con decimales y redondearlos correctamente.

from Ingreso.models import Ingreso  # Importa el modelo de ingresos del usuario.
from .strategy import (
    ObligacionesFiscalesStrategy,
)  # Importa una clase base que establece cómo se deben calcular las obligaciones fiscales.
from ..repositories import (
    ObligacionesFiscalesRepository,
)  # Importa algo que maneja cómo guardar información de las obligaciones fiscales.
...
class ArgentinaFiscalStrategy(ObligacionesFiscalesStrategy):
    ...
    def __init__(self, usuario):
        ...
        self.repositorio = ObligacionesFiscalesRepository(
            usuario
    ...
    def calcular_obligaciones(self, monotributo=False):
        ...
        if es_monotributista:
            ...
            if meses_registrados > 0:
                ingresos_anuales_estimados = (
                    ingresos_mensuales / meses_registrados
                ) * 12  # Estima los ingresos anuales a partir de los ingresos mensuales
                cuota_monotributo = self.calcular_cuota_monotributo(
                    ingresos_anuales_estimados
                )  # Calcula cuánto debe pagar de monotributo según sus ingresos anuales
                if cuota_monotributo:
                    vencimiento_monotributo = self.calcular_vencimiento_monotributo()
                    # Verificar si la obligación ya existe
                    if (
                        not self.repositorio.obtener_obligaciones_fiscales()
                        .filter(
            ...
            ganancias = self.calcular_retencion_ganancias(
                ingresos_mensuales
            )  # Calcula la retención de ganancias
            if ganancias:
                vencimiento_ganancias = (
                    self.calcular_vencimiento_ganancias()
                )  # Calcula la fecha de vencimiento para el pago de ganancias
                if (
                    not self.repositorio.obtener_obligaciones_fiscales()
                    .filter(
            ...
            if meses_registrados > 0:
                iva = self.calcular_iva(
                    ingresos_mensuales
                )  # Calcula el IVA sobre los ingresos mensuales
                if iva:
                    vencimiento_iva = (
                        self.calcular_vencimiento_iva()
                    )  # Calcula la fecha de vencimiento del IVA
                    if (
                        not self.repositorio.obtener_obligaciones_fiscales()
                        .filter(tipo_impuesto="IVA", fecha_vencimiento=vencimiento_iva)
                        .exists()
                    ):
                        self.repositorio.guardar_obligacion(
                            tipo_impuesto="IVA",
    ...
    def obtener_ingresos_mensuales(self):
        ...
        ingresos_totales = Ingreso.objects.filter(
            usuario_id=self.usuario, fecha__year=date.today().year
        )  # Filtra los ingresos del año actual
        if ingresos_totales.exists():  # Si hay ingresos registrados
            total_ingresos = sum(
                ingreso.monto for ingreso in ingresos_totales
            )  # Suma todos los ingresos
            meses_registrados = (
                self.contar_meses_registrados()
            )  # Cuenta los meses con ingresos
            if meses_registrados > 0:
                ingresos_mensuales_promedio = (
                    total_ingresos / meses_registrados
    ...
    def contar_meses_registrados(self):
        ...
        meses_registrados = len(
            set(ingreso.fecha.month for ingreso in ingresos_totales)
        )  # Crea un conjunto con los meses y cuenta cuántos diferentes hay
    ...
    def calcular_retencion_ganancias(self, ingresos_mensuales):
        ...
        for ingreso_bruto, importe in escala:
            if (
                ingresos_mensuales <= ingreso_bruto
            ):  # Si los ingresos mensuales caen en un tramo de la escala
                retencion = importe
                break
        else:
    ...
    def calcular_iva(self, ingresos_mensuales):
        ...
        )  # Redondea el valor del IVA a dos decimales usando la regla de redondeo estándar
    ...
    def calcular_vencimiento_ganancias(self):
        ...
        if date.today().day > 20:
            ...
            vencimiento = (date.today().replace(day=1) + timedelta(days=32)).replace(
                day=20
