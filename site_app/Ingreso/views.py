...
class IngresosPorMesView(APIView):
    ...
    def get(self, request, usuario_id, mes):
        ...
        ingresos_por_mes = self.ingresos_service.obtener_ingresos_de_un_mes(usuario_id, mes)
...
class IngresosPorAnioView(APIView):
    ...
    def get(self, request, usuario_id, anio):
        ...
        ingresos_por_anio = self.ingresos_service.obtener_ingresos_de_un_anio(usuario_id, anio)
...
class IngresoTotalPorMes(APIView):
    ...
    def get(self,request,usuario_id,mes,anio):
        ...
        ingresos_total = self.ingresos_service.obtener_ingreso_total_en_un_mes(usuario_id,mes,anio)
        if not ingresos_total:
            return Response({'message': 'No se encontraron ingresos para el usuario en el año indicado'}, status=status.HTTP_200_OK)
...
class IngresoTotalPorAnio(APIView):
    ...
    def get(self, request, usuario_id, anio):
        ...
        ingresos_total = self.ingresos_service.obtener_ingreso_total_en_un_anio(usuario_id, anio)
        if not ingresos_total:
            return Response({'message': 'No se encontraron ingresos para el usuario en el año indicado'}, status=status.HTTP_200_OK)
